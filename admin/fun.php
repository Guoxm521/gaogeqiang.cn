<?php
    ...
    */
    class Mysql {
        ...
        /* 
            需要完善   应该根据传来的属性来查，如分类，姓名这些，不仅仅是id
        */
        // 根据ID查找单个数据 
        public function selectByID($id) {
         ...
         // 分组查询
         public function selectBygroup($groupname) {
            $db = $this->connectdb($this->table);
            $sql = "select $groupname from $this->table group by $groupname";
            $query = $db->query($sql);
            $query->setFetchMode(PDO::FETCH_ASSOC);
            $result =$query->fetchAll();
            return $result;
        ...
        public function showpages($page,$pages,$beforepage,$afterpage,$params='') 
        {
            ...
            if ($pages == 1) {
                ...
                $i= $page <6 ? 1:$pages-5;
                if($page <6) {
                    $pagecount = $pages >=5 ? 5 : $pages;
                }else {
                    $pagecount = $pages>=5 ? $page : $pages;
                }
                // echo $i;
                for($i;$i<=$pagecount;$i++) {
                    if ($i == $page) {
